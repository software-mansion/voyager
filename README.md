...
- **`mix tauri.app` / `mix tauri.build` / CI** — same `.env` (or CI secrets) must be present **at compile time**. GitHub Actions pass `TELEMETRY_PUSH_URL` and `TELEMETRY_API_KEY` from repository secrets.
